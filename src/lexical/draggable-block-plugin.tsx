...
import { useEditorModal } from "@/lexical/use-modal";
...
export function DraggableBlockPlugin({
  ...
  function openComponentPicker(e: React.MouseEvent) {
    if (!draggableElement || !editor || !anchorElem) return;
    ...
    setPickerPosition(
      rect
        ? {
            left: rect.right - anchorRect.left + anchorElem.scrollLeft + 8,
            top: rect.top - anchorRect.top + anchorElem.scrollTop,
          }
        : null,
    );
    ...
    <>
      ...
      {isPickerOpen && pickerPosition
        ? ReactDOM.createPortal(
            <div
              ref={pickerRef}
              className="absolute z-50 w-56 rounded-md border border-border bg-popover shadow-md"
              style={{
